        ...
        if ($mensaje_estatus !== "") {
            ...
            if ($resUsuarios && $resUsuarios->num_rows > 0) {
                while ($user = $resUsuarios->fetch_assoc()) {
                    ...
                    $msg_whatsapp = "🚨 ALERTA UPGRADE SYSTEMS [{$mensaje_estatus}]: Hola {$user['nombre']}, el documento [{$doc['tipo_doc']}] está próximo a vencer el {$fecha_vence}. Por favor actualízalo.";
