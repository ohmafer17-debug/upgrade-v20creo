...
## SISTEMA DE GESTIÓN Y CONTROL DE VIGENCIA DE EXPEDIENTES CORPORATIVOS

Este documento detalla la arquitectura, estructura de datos, mecanismos de seguridad, automatización de alertas y guías de administración para los sistemas de gestión de expedientes **Upgrade Systems** (versión azul) y **Xonexka** (versión roja).
...
## 8. MANUAL DE INSTALACIÓN Y DESPLIEGUE LOCAL (XAMPP)

1.  **Requisitos Previos:**
    *   Instalar XAMPP con soporte para PHP 8.0 o superior.
2.  **Ubicación de Archivos:**
    *   Clonar o colocar la carpeta del proyecto en `C:\xampp\htdocs\upgrade_systems` (para la versión azul) o `C:\xampp\htdocs\upgrade_systems_xonexka` (para la versión roja).
...
4.  **Ejecutar la Aplicación:**
    ...
    *   Abrir en el navegador:
        *   **Portal Cliente (Azul):** `http://localhost:8080/upgrade_systems/public/cliente/login.html`
        *   **Portal Cliente (Rojo Xonexka):** `http://localhost:8080/upgrade_systems_xonexka/public/cliente/login.html`
        *   **Administración (Azul):** `http://localhost:8080/upgrade_systems/public/admin/login.html`
        *   **Administración (Rojo Xonexka):** `http://localhost:8080/upgrade_systems_xonexka/public/admin/login.html`
5.  **Configurar Tareas Programadas (Alertas en Producción):**
    ...
    *   En Windows local, configurar una Tarea Programada que llame al ejecutable de PHP:
        `C:\xampp\php\php.exe -f C:\xampp\htdocs\upgrade_systems\controllers\alertas_automaticas.php`
