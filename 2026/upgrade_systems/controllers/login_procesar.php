...
if ($resAdmin && $resAdmin->num_rows > 0) {
    ...
    if (password_verify($pass_post, $rowAdmin['pass']) || $pass_post === $rowAdmin['pass']) {
        // Exitoso: resetear intentos y guardar variables de sesión nativas de PHP
        $_SESSION['login_attempts'] = 0;
        unset($_SESSION['login_lock_until']);
        $_SESSION['ups_sesion_id'] = "UPS-STAFF";
        $_SESSION['usuario_sesion_email'] = $rowAdmin['email'];
        $_SESSION['usuario_sesion_rol'] = $rowAdmin['rol'];
...
if ($resCliente && $resCliente->num_rows > 0) {
    ...
    if (password_verify($pass_post, $rowCliente['pass']) || $pass_post === $rowCliente['pass']) {
        ...
        $_SESSION['login_attempts'] = 0;
        unset($_SESSION['login_lock_until']);
        $_SESSION['cliente_sesion_id'] = $rowCliente['cod'];
        $_SESSION['usuario_sesion_email'] = $rowCliente['email'];
        $_SESSION['usuario_sesion_rol'] = $rol_real_bd;
