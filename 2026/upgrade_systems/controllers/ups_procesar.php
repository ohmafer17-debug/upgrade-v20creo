...
if (in_array($accion, $acciones_empresas_mutar)) {
    if ($rol_ejecutor !== 'Administrador' && $rol_ejecutor !== 'Usuario Estándar') {
        echo json_encode(["status" => "error", "message" => "Acción denegada: Tu rol de '{$rol_ejecutor}' (ejecutor: '{$usuario_ejecutor_email}') no tiene permisos para esta acción."]);
        exit;
    }
}
...
if (in_array($accion, $acciones_staff_mutar)) {
    if ($rol_ejecutor !== 'Administrador') {
        echo json_encode(["status" => "error", "message" => "Acción denegada: Únicamente el Administrador Máster (tu rol: '{$rol_ejecutor}', ejecutor: '{$usuario_ejecutor_email}') puede gestionar el Staff interno."]);
        exit;
    }
}
...
if ($accion === 'registrar_nueva_empresa') {
    ...
    $dueno_nombre       = isset($_POST['dueno_nombre']) ? trim($_POST['dueno_nombre']) : (isset($datos['dueno_nombre']) ? trim($datos['dueno_nombre']) : '');
    $dueno_email        = isset($_POST['dueno_email']) ? trim($_POST['dueno_email']) : (isset($datos['dueno_email']) ? trim($datos['dueno_email']) : '');
    ...
    // Insertar incluyendo el logo y los nuevos datos de Dueño/Director
    $stmt = $conexion->prepare("INSERT INTO empresas_clientes (cod, nombre, email, email_adicional, telefono_principal, telefono_adicional, dueño_director_nombre, dueño_director_email, direccion, coordenadas, pass, activo, rol, logo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssssssssss", $empresa_cod, $empresa_nombre, $email_usuario, $email_adicional, $telefono_principal, $telefono_adicional, $dueno_nombre, $dueno_email, $direccion, $coordenadas, $pass_encriptada, $activo_inicial, $rol_inicial, $logo_nombre_fisico);
...
if ($accion === 'listar_licencias_globales') {
    ...
    if ($filtro === 'TODAS') {
        $query = "SELECT id, cod, nombre, email, email_adicional, telefono_principal, telefono_adicional, dueño_director_nombre, dueño_director_email, direccion, coordenadas, rol, activo, logo FROM empresas_clientes ORDER BY $ordenJerarquico, id DESC";
        $res = $conexion->query($query);
    } else {
        // Sentencia preparada para el filtrado dinámico por sucursal
        $filtro_like = $filtro . "/%";
        $stmt = $conexion->prepare("SELECT id, cod, nombre, email, email_adicional, telefono_principal, telefono_adicional, dueño_director_nombre, dueño_director_email, direccion, coordenadas, rol, activo, logo FROM empresas_clientes WHERE cod = ? OR cod LIKE ? ORDER BY $ordenJerarquico, id DESC");
    ...
    if ($res) {
        while ($row = $res->fetch_assoc()) { 
            $licencias[] = [
                ...
                "dueño_director_nombre" => $row['dueño_director_nombre'],
                "dueño_director_email" => $row['dueño_director_email'],
...
if ($accion === 'editar_empresa_cliente') {
    ...
    $query = "UPDATE empresas_clientes SET 
                ...
                telefono_principal = ?, 
                telefono_adicional = ?, 
                dueño_director_nombre = ?, 
                dueño_director_email = ?, 
                direccion = ?, 
                coordenadas = ?, 
                rol = ?";
    ...
    if ($pass_hash !== null && $logo_actualizado) {
        $stmt->bind_param("sssssssssssssi", $nombre, $email, $email_adicional, $telefono_principal, $telefono_adicional, $dueno_nombre, $dueno_email, $direccion, $coordenadas, $rol, $pass_hash, $logo_nombre_fisico, $id);
    } elseif ($pass_hash !== null) {
        $stmt->bind_param("sssssssssssi", $nombre, $email, $email_adicional, $telefono_principal, $telefono_adicional, $dueno_nombre, $dueno_email, $direccion, $coordenadas, $rol, $pass_hash, $id);
    } elseif ($logo_actualizado) {
        $stmt->bind_param("ssssssssssss", $nombre, $email, $email_adicional, $telefono_principal, $telefono_adicional, $dueno_nombre, $dueno_email, $direccion, $coordenadas, $rol, $logo_nombre_fisico, $id);
    } else {
        $stmt->bind_param("ssssssssssi", $nombre, $email, $email_adicional, $telefono_principal, $telefono_adicional, $dueno_nombre, $dueno_email, $direccion, $coordenadas, $rol, $id);
    }
