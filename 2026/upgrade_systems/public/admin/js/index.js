...
let cacheLicencias = [];
let cacheAdmins = [];
let ordenColumnaLicencias = '';
let ordenDireccionLicencias = 'asc';
let ordenColumnaAdmins = '';
let ordenDireccionAdmins = 'asc';
let mapaRegistro, markerRegistro;
let mapaEdicion, markerEdicion;
...
document.getElementById('registroEmpresaForm').addEventListener('submit', async function(e) {
    ...
    formData.append('dueno_nombre', document.getElementById('dueñoNombre').value.trim());
    formData.append('dueno_email', document.getElementById('dueñoEmail').value.trim());
...
function ordenarLicencias(columna) {
    if (ordenColumnaLicencias === columna) {
        ordenDireccionLicencias = ordenDireccionLicencias === 'asc' ? 'desc' : 'asc';
    } else {
        ordenColumnaLicencias = columna;
        ordenDireccionLicencias = 'asc';
    }

    // Resetear iconos de ordenamiento en la tabla de licencias
    document.querySelectorAll('#vista-licencias .sortable-headers th i').forEach(icon => {
        icon.className = 'fas fa-sort';
        icon.style.opacity = '0.4';
    });

    const thIndex = { 'cod': 0, 'nombre': 1, 'email': 2 }[columna];
    const headers = document.querySelectorAll('#vista-licencias .sortable-headers th');
    if (headers && headers[thIndex]) {
        const icon = headers[thIndex].querySelector('i');
        if (icon) {
            icon.className = ordenDireccionLicencias === 'asc' ? 'fas fa-sort-up' : 'fas fa-sort-down';
            icon.style.opacity = '0.9';
        }
    }

    cacheLicencias.sort((a, b) => {
        let valA = '';
        let valB = '';
        ...
        if (columna === 'cod') {
            ...
            valA = a.nombre_comercial || '';
            valB = b.nombre_comercial || '';
        } else if (columna === 'email') {
            valA = a.usuario_responsable || '';
            valB = b.usuario_responsable || '';
        }

        valA = valA.toString().toLowerCase();
        valB = valB.toString().toLowerCase();

        if (valA < valB) return ordenDireccionLicencias === 'asc' ? -1 : 1;
        if (valA > valB) return ordenDireccionLicencias === 'asc' ? 1 : -1;
        return 0;
    });
...
function renderizarTablaLicencias() {
    const t = document.getElementById('tablaLicenciasBody'); if (!t) return;
    t.innerHTML = "";
    const rolActive = localStorage.getItem('cliente_sesion_rol') || 'Invitado';

    cacheLicencias.forEach((emp) => {
        ...
        if (rolActive !== 'Invitado') {
            accionesHtml += `<button class="btn-action edit" onclick="prepararEdicionEmpresa('${emp.id}')"><i class="fas fa-edit"></i> Editar</button>`;
...
function prepararEdicionEmpresa(id) {
    const emp = cacheLicencias.find(e => e.id == id);
    ...
    document.getElementById('editDueñoNombre').value = emp.dueño_director_nombre || '';
    document.getElementById('editDueñoEmail').value = emp.dueño_director_email || '';
    ...
    // 🗺️ Actualizar el tamaño del mapa de edición y mover el Pin
    setTimeout(() => {
        if (mapaEdicion && markerEdicion) {
            mapaEdicion.invalidateSize();
            if (emp.coordenadas) {
                const parts = emp.coordenadas.split(',').map(c => c.trim());
                if (parts.length === 2) {
                    const lat = parseFloat(parts[0]);
                    const lon = parseFloat(parts[1]);
                    if (!isNaN(lat) && !isNaN(lon)) {
                        mapaEdicion.setView([lat, lon], 16);
                        markerEdicion.setLatLng([lat, lon]);
                    }
                }
            }
        }
...
document.getElementById('edicionEmpresaForm').addEventListener('submit', async function(e) {
    ...
    formData.append('dueno_nombre', document.getElementById('editDueñoNombre').value.trim());
    formData.append('dueno_email', document.getElementById('editDueñoEmail').value.trim());
...
async function cargarAdministradoresUps() {
    ...
    const res = await r.json();
    if (res.status === 'success') {
        cacheAdmins = res.data;
        renderizarTablaAdmins();
    }
}

function ordenarAdmins(columna) {
    if (ordenColumnaAdmins === columna) {
        ordenDireccionAdmins = ordenDireccionAdmins === 'asc' ? 'desc' : 'asc';
    } else {
        ordenColumnaAdmins = columna;
        ordenDireccionAdmins = 'asc';
    }

    // Resetear iconos de ordenamiento en la tabla de admins
    document.querySelectorAll('#vista-admins .sortable-headers th i').forEach(icon => {
        icon.className = 'fas fa-sort';
        icon.style.opacity = '0.4';
    });

    const thIndex = { 'id': 0, 'nombre': 1, 'contacto': 2, 'rol': 3, 'estatus': 4 }[columna];
    const headers = document.querySelectorAll('#vista-admins .sortable-headers th');
    if (headers && headers[thIndex]) {
        const icon = headers[thIndex].querySelector('i');
        if (icon) {
            icon.className = ordenDireccionAdmins === 'asc' ? 'fas fa-sort-up' : 'fas fa-sort-down';
            icon.style.opacity = '0.9';
        }
    }

    cacheAdmins.sort((a, b) => {
        let valA = '';
        let valB = '';
        ...
        if (columna === 'id') {
            valA = parseInt(a.id);
            valB = parseInt(b.id);
            return ordenDireccionAdmins === 'asc' ? valA - valB : valB - valA;
        } else if (columna === 'nombre') {
            valA = a.nombre || '';
            valB = b.nombre || '';
        } else if (columna === 'contacto') {
            valA = a.email || '';
            valB = b.email || '';
        } else if (columna === 'rol') {
            valA = a.rol || '';
        ...
        if (valA < valB) return ordenDireccionAdmins === 'asc' ? -1 : 1;
        if (valA > valB) return ordenDireccionAdmins === 'asc' ? 1 : -1;
        return 0;
    });
...
function renderizarTablaAdmins() {
    const t = document.getElementById('tablaAdminsBody'); if (!t) return;
    t.innerHTML = "";
    const emailLogueado = localStorage.getItem('ups_sesion_email') || '';

    cacheAdmins.forEach(adm => {
...
// 🚀 GEOCODIFICACIÓN DINÁMICA BIDIRECCIONAL (OPENSTREETMAP NOMINATIM CON MAPAS LEAFLET)
async function geocodificarDireccion(idDireccion, idCoordenadas) {
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            // 🗺️ Actualizar el mapa correspondiente
            const latFloat = parseFloat(lat);
            const lonFloat = parseFloat(lon);
            if (idDireccion === 'editEmpresaDireccion') {
                if (mapaEdicion && markerEdicion) {
                    mapaEdicion.setView([latFloat, lonFloat], 16);
                    markerEdicion.setLatLng([latFloat, lonFloat]);
                }
            } else {
                if (mapaRegistro && markerRegistro) {
                    mapaRegistro.setView([latFloat, lonFloat], 16);
                    markerRegistro.setLatLng([latFloat, lonFloat]);
                }
            }
...
async function geocodificarCoordenadas(idCoordenadas, idDireccion) {
    ...
    // 🗺️ Mover mapa y Pin al escribir coordenadas manualmente
    if (idCoordenadas === 'editEmpresaCoordenadas') {
        if (mapaEdicion && markerEdicion) {
            mapaEdicion.setView([lat, lon], 16);
            markerEdicion.setLatLng([lat, lon]);
        }
        ...
        if (mapaRegistro && markerRegistro) {
            mapaRegistro.setView([lat, lon], 16);
            markerRegistro.setLatLng([lat, lon]);
        }
    }
...
function inicializarMapas() {
    const latDefault = 19.432608;
    const lngDefault = -99.133208;

    // 1. Mapa de Registro
    const containerReg = document.getElementById('mapa-registro');
    if (containerReg) {
        mapaRegistro = L.map('mapa-registro').setView([latDefault, lngDefault], 13);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '© OpenStreetMap contributors'
        }).addTo(mapaRegistro);

        markerRegistro = L.marker([latDefault, lngDefault], { draggable: true }).addTo(mapaRegistro);

        // Al arrastrar el pin
        markerRegistro.on('dragend', function (e) {
            const position = markerRegistro.getLatLng();
            document.getElementById('empresaCoordenadas').value = `${position.lat.toFixed(6)}, ${position.lng.toFixed(6)}`;
            geocodificarCoordenadas('empresaCoordenadas', 'empresaDireccion');
        });

        // Al hacer doble clic en el mapa
        mapaRegistro.on('dblclick', function (e) {
            const position = e.latlng;
            markerRegistro.setLatLng(position);
            document.getElementById('empresaCoordenadas').value = `${position.lat.toFixed(6)}, ${position.lng.toFixed(6)}`;
            geocodificarCoordenadas('empresaCoordenadas', 'empresaDireccion');
        });
        mapaRegistro.doubleClickZoom.disable();
    }

    // 2. Mapa de Edición
    const containerEd = document.getElementById('mapa-edicion');
    if (containerEd) {
        mapaEdicion = L.map('mapa-edicion').setView([latDefault, lngDefault], 13);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '© OpenStreetMap contributors'
        }).addTo(mapaEdicion);

        markerEdicion = L.marker([latDefault, lngDefault], { draggable: true }).addTo(mapaEdicion);

        // Al arrastrar el pin
        markerEdicion.on('dragend', function (e) {
            const position = markerEdicion.getLatLng();
            document.getElementById('editEmpresaCoordenadas').value = `${position.lat.toFixed(6)}, ${position.lng.toFixed(6)}`;
            geocodificarCoordenadas('editEmpresaCoordenadas', 'editEmpresaDireccion');
        });

        // Al hacer doble clic en el mapa
        mapaEdicion.on('dblclick', function (e) {
            const position = e.latlng;
            markerEdicion.setLatLng(position);
            document.getElementById('editEmpresaCoordenadas').value = `${position.lat.toFixed(6)}, ${position.lng.toFixed(6)}`;
            geocodificarCoordenadas('editEmpresaCoordenadas', 'editEmpresaDireccion');
        });
        mapaEdicion.doubleClickZoom.disable();
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 🗺️ Inicializar mapas de Leaflet
    inicializarMapas();
